...
import { ChevronRight, ChevronLeft, Check, ImagePlus, Sparkles } from 'lucide-react';
...
export function VisionWizard() {
  ...
  const generatePlanDraft = useStore((s) => s.generatePlanDraft);

  const commitVisionWithPlan = useStore((s) => s.commitVisionWithPlan);
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      const result = await generatePlanDraft(wizardData());
  ...
  const handleValidatePlan = async (items: DraftPlanItem[]) => {

    setCommitting(true);

    try {

      await commitVisionWithPlan(currentSpace, wizardData(), items);
            ...
            {step === 0 && (
              ...
              <div className="space-y-5">
                ...
                <p className="text-xs text-aw-muted leading-relaxed rounded-xl px-3 py-2.5" style={{ backgroundColor: 'var(--aw-warm)' }}>
                  ...
                  L'IA rétro-planifie depuis votre point de départ : 2 jalons annuels, 4 semestriels,

                  8 trimestriels, 24 mensuels, plus les actions de la semaine (dim.–ven.) et les blocs horaires du jour.

                  Le sabbat (ven. soir → sam. soir) est respecté — aucune tâche planifiée pendant ce temps.
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch gap-3 mt-6 pt-5 border-t border-aw-line">
          ...
          {step < STEPS.length - 1 ? (
            ...
            >
              ...
              {generating ? 'L\'IA génère votre plan…' : 'Générer le plan d\'action par IA'}
